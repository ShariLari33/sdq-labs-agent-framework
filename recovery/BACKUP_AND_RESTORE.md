...
- Hermes state/config location metadata

Backups do not include plaintext `.env` files by default. Secrets must be stored separately in a password manager.

Hermes `.local/hermes/venv` is regenerated by `make hermes-install`. Repository-local Hermes state/config under `.local/hermes/state` and `.local/hermes/config` may contain runtime memory or provider configuration; review and back it up separately only after removing secrets or encrypting the backup.
